...
class BufferLine with IndexedItem {
  ...
  int getTrimmedLength([int? cols]) {
    final maxCols = min(_length, _data.length ~/ _cellSize);
    ...
    for (var i = cols - 1; i >= 0; i--) {
      ...
      if (codePoint != 0) {
        ...
        return min(i + lastCellWidth, cols);
      }
    }
    return 0;
  }
  ...
  void copyFrom(BufferLine src, int srcCol, int dstCol, int len) {
    if (srcCol < 0 || dstCol < 0 || len <= 0) {
      return;
    }
    final readableCells =
        min(src._length, src._data.length ~/ _cellSize) - srcCol;
    if (readableCells <= 0) {
      return;
    }
