...
class AiOpenAiPromptCacheControl {
  const AiOpenAiPromptCacheControl._();

  static const String optionsBodyField = 'prompt_cache_options';
  static const String legacyRetentionBodyField = 'prompt_cache_retention';
  static const String breakpointField = 'prompt_cache_breakpoint';
  static const Map<String, String> _options = <String, String>{
    'mode': 'implicit',
    'ttl': '30m',
  };
  static const Map<String, String> _breakpoint = <String, String>{
    'mode': 'explicit',
  };
  ...
  static Map<String, Object?> applyToBody({
    ...
    if (!AiModelConfig.supportsOpenAiPromptCacheBreakpointsFor(
          modelId: '${body['model'] ?? ''}',
    ...
    final conversationField = body.containsKey('messages')
        ? 'messages'
        : body.containsKey('input')
        ? 'input'
        : null;
    final updated = Map<String, Object?>.from(body)
      ..remove(legacyRetentionBodyField)
      ..[optionsBodyField] = _options;
    if (conversationField == null || body[conversationField] is! List) {
      return updated;
    }
    final responsesInput = conversationField == 'input';
    final conversation = (body[conversationField] as List)
        .map(
          (item) => item is Map
              ? Map<String, Object?>.from(stringKeyedMapFromValue(item))
              : item,
        )
        .toList(growable: false);
    final leadingSystemMessages = <Map<String, Object?>>[];
    for (final item in conversation) {
      if (item is! Map<String, Object?> || item['role'] != 'system') break;
      leadingSystemMessages.add(item);
    }
    if (leadingSystemMessages.isNotEmpty) {
      final breakpointMessages = <Map<String, Object?>>[];
      final stablePrefixMessageCount =
          inputCacheConfig.stablePrefixMessageCount;
      if (responsesInput &&
          stablePrefixMessageCount > 0 &&
          stablePrefixMessageCount <= leadingSystemMessages.length) {
        breakpointMessages.add(
          leadingSystemMessages[stablePrefixMessageCount - 1],
        );
      }
      final lastLeadingSystemMessage = leadingSystemMessages.last;
      if (breakpointMessages.isEmpty ||
          !identical(breakpointMessages.last, lastLeadingSystemMessage)) {
        breakpointMessages.add(lastLeadingSystemMessage);
      }
      final breakpointBudget = inputCacheConfig.breakpointCount.clamp(1, 4);
      for (final message in breakpointMessages.take(breakpointBudget)) {
        _attachBreakpoint(message, responsesInput: responsesInput);
      }
    }
    updated[conversationField] = conversation;
    return updated;
  }

  static bool bodyHasMarkers(Map<String, Object?> body) =>
      body.containsKey(optionsBodyField) ||
      body.containsKey(legacyRetentionBodyField) ||
      _containsBreakpoint(body);
  ...
  static bool shouldRetryWithoutMarkers({
    ...
    if ((statusCode != 400 && statusCode != 422) ||
        !bodyHasMarkers(requestBody)) {
      return false;
    }
    final normalized = errorBody.toLowerCase();
    return normalized.contains(optionsBodyField) ||
        normalized.contains(legacyRetentionBodyField) ||
        normalized.contains(breakpointField) ||
        normalized.contains('invalid') ||
        normalized.contains('schema') ||
        normalized.contains('unsupported') ||
        normalized.contains('unrecognized') ||
  ...
  static Map<String, Object?> withoutMarkers(Map<String, Object?> body) {
    if (!bodyHasMarkers(body)) return body;
    final sanitized = _withoutBreakpoint(body) as Map<String, Object?>;
    sanitized
  ...
  static void _attachBreakpoint(
    Map<String, Object?> message, {
    required bool responsesInput,
  }) {
    final content = message['content'];
    if (content is String && content.trim().isNotEmpty) {
      message['content'] = <Map<String, Object?>>[
        <String, Object?>{
          'type': responsesInput ? 'input_text' : 'text',
          'text': content,
          breakpointField: _breakpoint,
        },
      ];
      return;
    }
    if (content is! List) return;
    final mutableContent = content.toList(growable: false);
    final supportedTypes = responsesInput
        ? const <String>{'input_text', 'input_image', 'input_file'}
        : const <String>{'text', 'image_url', 'input_audio', 'file', 'refusal'};
    for (var index = mutableContent.length - 1; index >= 0; index -= 1) {
      final item = mutableContent[index];
      if (item is! Map) continue;
      final block = Map<String, Object?>.from(stringKeyedMapFromValue(item));
      if (!supportedTypes.contains('${block['type'] ?? ''}')) continue;
      block[breakpointField] = _breakpoint;
      mutableContent[index] = block;
      message['content'] = mutableContent;
      return;
    }
  }

  static bool _containsBreakpoint(Object? value) {
    if (value is Map) {
      if (value.containsKey(breakpointField)) return true;
      return value.values.any(_containsBreakpoint);
    }
    return value is List && value.any(_containsBreakpoint);
  }

  static Object? _withoutBreakpoint(Object? value) {
    if (value is Map) {
      final result = <String, Object?>{};
      for (final entry in value.entries) {
        final key = '${entry.key}';
        if (key == breakpointField) continue;
        result[key] = _withoutBreakpoint(entry.value);
      }
      return result;
    }
    if (value is List) {
      return value.map(_withoutBreakpoint).toList(growable: false);
    }
    return value;
  }
}
