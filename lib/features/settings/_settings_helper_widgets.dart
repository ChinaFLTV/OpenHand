...
/// 用法：包一层 `_KeyTweakableSlider(value:..., min:..., max:..., onChanged:...)`，
/// `_buildSlider` 闭包负责把 value 透传到内部 [Slider]，避免每个调用点都重写 Focus
/// + onKeyEvent 样板。
class _KeyTweakableSlider extends StatefulWidget {
  const _KeyTweakableSlider({
    required this.value,
    required this.min,
    required this.max,
  ...
  /// 调用方写到磁盘的入口；返回 false 走 SnackBar，true 静默成功。
  /// 这里不关心返回值——失败提示由调用方 Slider.onChanged 闭包内自己处理；
  /// 微调路径直接 fire-and-forget。
  ...
  @override
  State<_KeyTweakableSlider> createState() => _KeyTweakableSliderState();
}

class _KeyTweakableSliderState extends State<_KeyTweakableSlider> {
  final FocusNode _focus = FocusNode(debugLabel: 'settings.slider.tweakable');
  ...
  KeyEventResult _handleKey(FocusNode _, KeyEvent event) {
    ...
    if (event.logicalKey == LogicalKeyboardKey.arrowLeft ||
      ...
      delta = -_KeyTweakableSlider._step;
...
/// 把 [child] 套上一个「首帧后自动 pulse 一次」的高亮条。专用于 Phase 14
/// Task 5：用户首次滚到/打开 Settings 的 MCP 段落时，让两条新参数行短暂
/// 高亮（reduceMotion 下被 [HighlightPulse] 自动跳过，不会闪烁）。
///
/// 触发策略：StatefulWidget 的 [State.initState] 注册一次 postFrameCallback，
/// 在下一帧 mounted 时自增 [_pulse]；HighlightPulse 监听到自增即 fadeIn+decay。
/// 同一个 Widget 实例只会 pulse 一次；切换 tab/重新打开 Settings 会重新生成
/// State，因此每次进入页面都会高亮一次。
class _FirstFramePulseBox extends StatefulWidget {
  const _FirstFramePulseBox({required this.child});

  final Widget child;

  @override
  State<_FirstFramePulseBox> createState() => _FirstFramePulseBoxState();
}

class _FirstFramePulseBoxState extends State<_FirstFramePulseBox> {
