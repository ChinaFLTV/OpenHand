...
class _McpViewState extends State<McpView> with WidgetsBindingObserver {
    ...
    final actions = FeaturePageToolbar(
      ...
      secondaryActions: [
        FeaturePageToolbarIconButton(
          tooltip: openHandMcpMarketLabel(context),
          ...
          onActionSelected: (action) {
            switch (action) {
              ...
              case _McpCardAction.viewHistory:
                _showHealthHistoryDialog(context, server.name);
  ...
  /// 弹出标准弹窗展示该服务的最近 30 条探测历史，用 Selector 监听 controller
  /// 的健康表，使得 reconnect / 自动健康检查刷新历史时内容会自动跟新。
  void _showHealthHistoryDialog(BuildContext context, String serverName) {
    showAnimatedDialog<void>(
      context: context,
      builder: (_) {
        return Selector<McpController, McpServerHealth>(
          selector: (_, controller) => controller.healthStatusFor(serverName),
          builder: (context, health, _) =>
              _McpHealthHistoryDialog(serverName: serverName, health: health),
        );
      },
    );
  }
              ...
              children: [
                ...
                _McpHorizontalChipStrip(
                  children: [
                    ...
                    for (final templateId in unknownVisibleTemplateIds)
                      _McpStatusChip(
                        key: ValueKey<String>('mcp-chip-template-$templateId'),
                        icon: Icons.extension_rounded,
                        color: colorScheme.tertiary,
                        label: templateId,
                      ),
                ...
                OpenHandMetricsStrip(
                  items: [
                    ...
                    (
                      label: _localizedText(context, zh: 'Tools', en: 'Tools'),
                    ...
                    (
                      ...
                      value: healthStatus.isChecking
                          ...
                          : healthStatus.lastCheckedAt == null
                          ? _localizedText(context, zh: '未检测', en: 'Unchecked')
...
class _McpServerDetailsDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final colorScheme = theme.colorScheme;
    final probes = health.recentProbes;
    final stats = _mcpProbeStats(probes);
    final lastFailureAt = probes
        .where((p) => p.status != McpServerHealthStatus.healthy)
        .firstOrNull
        ?.timestamp;

    final healthLabel = switch (health.status) {
    ...
    final healthColor = _healthStatusDotColor(
      colorScheme,
      server: server,
      healthStatus: health,
    );
    final catalogStatus = toolCatalog.isLoading
    ...
    final catalogColor = toolCatalog.isLoading
        ? OpenHandStatusColors.info
        : toolCatalog.errorMessage != null
        ? colorScheme.error
        : OpenHandStatusColors.success;

    final lastSuccessRelative = health.lastSuccessAt == null
        ? null
        : _formatRelativePast(context, health.lastSuccessAt!);
    final lastFailureRelative = lastFailureAt == null
        ? null
...
class _ProbeTrendSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasAnyLatency = ordered.any(
      (probe) =>
          probe.status == McpServerHealthStatus.healthy &&
        ...
        Row(
          children: [
            ...
            if (!hasAnyLatency) ...[
              kOpenHandHGap12,
              Text(
                _localizedText(context, zh: '暂无耗时样本', en: 'No latency samples'),
                style: theme.textTheme.labelSmall?.copyWith(
                  color: colorScheme.onSurfaceVariant,
                ),
              ),
...
/// 「最近探测历史」弹窗：渲染最多 30 条 [McpHealthProbeRecord]，按时间倒序，
/// 健康记录显示绿色对勾 + 耗时，失败记录显示红色叹号 + 截断错误信息。
class _McpHealthHistoryDialog extends StatelessWidget {
  const _McpHealthHistoryDialog({
  ...
  Widget build(BuildContext context) {
    ...
    final probes = health.recentProbes;
    final stats = _mcpProbeStats(probes);
    final copyMenu = AnimatedPopupMenuButton<_McpHistoryExportFormat>(
      tooltip: _localizedText(context, zh: '复制探测历史', en: 'Copy probe history'),
      style: openHandFeatureCircleIconButtonStyle(colorScheme),
      icon: const Icon(Icons.copy_all_rounded),
      onSelected: (format) => _copyHistoryToClipboard(context, format),
      itemBuilder: (context) => [
        PopupMenuItem(
          value: _McpHistoryExportFormat.markdown,
          child: Text(
            _localizedText(context, zh: '复制为 Markdown', en: 'Copy as Markdown'),
          ),
        ),
        PopupMenuItem(
          value: _McpHistoryExportFormat.json,
          child: Text(
            _localizedText(context, zh: '复制为 JSON', en: 'Copy as JSON'),
          ),
        ),
        PopupMenuItem(
          value: _McpHistoryExportFormat.csv,
          child: Text(
            _localizedText(context, zh: '复制为 CSV', en: 'Copy as CSV'),
          ),
        ),
      ],
    );
        ...
        children: [
          ...
          if (probes.isEmpty)
            ...
            Padding(
              ...
              child: OpenHandMetricsStrip(
                items: [
                  (
                    label: _localizedText(context, zh: '记录数', en: 'Samples'),
                    value: '${probes.length}',
                    accent: colorScheme.primary,
                  ),
                  (
                    label: _localizedText(context, zh: '成功', en: 'Healthy'),
...
class _McpHealthProbeTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final colorScheme = theme.colorScheme;
    final isHealthy = probe.status == McpServerHealthStatus.healthy;
    final accentColor = isHealthy
        ? OpenHandStatusColors.success
        : colorScheme.error;
    final errorText = probe.errorMessage?.trim() ?? '';
    final relative = _formatRelativePast(context, probe.timestamp);
    final statusText = isHealthy
        ? _localizedText(context, zh: '健康', en: 'Healthy')
        : _localizedText(context, zh: '失败', en: 'Failed');
    final latencyValue = probe.latencyMs == null ? '—' : '${probe.latencyMs}';
    final latencyUnit = probe.latencyMs == null
        ? _localizedText(context, zh: '无耗时', en: 'No latency')
        : 'ms';
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      kOpenHandGap6,
                      Text(
                        formatYearMonthDayHmsLocal(probe.timestamp),
                        style: theme.textTheme.bodySmall?.copyWith(
                          color: colorScheme.onSurfaceVariant,
                        ),
                      ),
