...
class _ConnectivityResultView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final summary = openHandConnectivityProbeSummary(
      text: text,
...
Color _connectivityLogLineColor(String line) {
  final lower = line.toLowerCase();
  if (line.contains('未通过') ||
      line.contains('失败') ||
      line.contains('無效') ||
      line.contains('无效') ||
...
IconData _connectivityFieldIcon(String key) {
...
Color _connectivityFieldAccent(
  ...
  switch (lower) {
    ...
    case 'state':
      return switch (raw) {
        'running' || 'ready' || 'idle' => OpenHandStatusColors.success,
        'crashed' || 'error' || 'failed' => OpenHandStatusColors.error,
        'stopped' || 'stopping' || 'degraded' => OpenHandStatusColors.warning,
...
String _connectivityFieldValue(
  BuildContext context, {
  required String key,
  required Object? value,
}) {
  if (value == null) return openHandJsonScalarLabel(context, 'null');
  if (value is bool) {
    return openHandJsonScalarLabel(context, value.toString());
  }
  if (value is num) {
    final lower = key.trim().toLowerCase();
    if (lower.endsWith('_ms') || lower == 'duration' || lower == 'latency') {
      return openHandMillisecondsLabel(context, value.round());
    }
    return '$value';
  }
  if (value is String) {
    final trimmed = value.trim();
    if (trimmed.isEmpty) return openHandJsonScalarLabel(context, 'null');
    if (trimmed == webMessagePlatformBuiltinName) {
      return openHandLocalizedText(
        context,
        zh: 'Web 通用消息平台',
        zhHant: 'Web 通用訊息平台',
        en: 'Web message platform',
        fr: 'Passerelle de messages web',
        de: 'Web-Nachrichtendienst',
        ja: 'Webメッセージプラットフォーム',
      );
    }
    final parsedTime = utcDateTimeFromValue(trimmed);
    final lower = key.trim().toLowerCase();
    if (parsedTime != null &&
        (lower.contains('time') ||
            lower.contains('_at') ||
            trimmed.contains('T'))) {
      return formatYearMonthDayHmsLocal(parsedTime.toLocal());
    }
    return openHandJsonScalarLabel(context, trimmed);
  }
  return _formatStructuredValue(value);
}
...
class _MetricTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final colorScheme = theme.colorScheme;
    final tone = colorScheme.primary;
    return AnimatedContainer(
      duration: openHandMotionDurationMs(context, 180),
      curve: kOpenHandSwitchInCurve,
      padding: const EdgeInsets.all(14),
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                final bubbleAlignment = widget.mine
                    ? Alignment.topRight
                    : Alignment.topLeft;
                final bubbleContent = DingTalkMessageContentTransition(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: colors.primary.withValues(
                    alpha: enabled && widget.highlighted ? 0.82 : 0,
                  ),
                  width: 1.8,
                ),
              ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                kOpenHandGap5,
                DingTalkMessageContentTransition(
                  expanded: contentExpanded,
                  child: !contentExpanded
                      ? KeyedSubtree(
                            ...
                            children: [
                              if (media.isNotEmpty)
                                AnimatedOpacity(
                                  ...
                                  curve: kOpenHandSwitchInCurve,
                                  opacity: item.ignoredForAiContext ? 0.68 : 1,
                                ...
                                Container(
                                  ...
                                  child: _DingTalkLinkifiedText(
                                    ...
                                    onOpenLink: (href) {
                                      _cancelPendingActionToggle();
                                      unawaited(
                                        _openDingTalkMessageLink(context, href),
                                      );
                                    },
                                  ),
                                ),
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<Object?>(
      ...
      builder: (context, snapshot) {
        final theme = Theme.of(context);
        final colorScheme = theme.colorScheme;
        final stateKey = snapshot.connectionState != ConnectionState.done
            ? 'loading'
            : snapshot.hasError
            ? 'error'
            : snapshot.data == null
...
class _DingTalkDetailsViewState extends State<_DingTalkDetailsView> {
  ...
  Widget build(BuildContext context) {
    ...
    final conversationFacts = dingTalkDetailOmitIdentityEcho(
    ...
    final sections = <Widget>[
      ...
      if (conversationFacts.isNotEmpty)
        _DingTalkDetailCardGroup(
          ...
          badge: _dingtalkDetailCountLabel(
            context,
            dingTalkDetailVisibleCount(conversationFacts),
          ),
          child: _DingTalkDetailGrid(data: conversationFacts),
        ),
...
class _DingTalkDetailCardGroup extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final accent = _dingtalkDetailSectionAccent(context, title);
    ...
    return Column(
      ...
      children: [
        ...
        if (simpleEntries.isNotEmpty) ...[
          ...
          _dingtalkDetailFactSheet(context, simpleEntries),
        ],
        if (compoundEntries.isNotEmpty) ...[
          ...
          for (var index = 0; index < compoundEntries.length; index++) ...[
            ...
            _DingTalkDetailNestedSection(
              label: compoundEntries[index].key,
              value: compoundEntries[index].value,
            ),
          ],
        ],
      ],
                ...
                child: Row(
                  ...
                  children: [
                    Padding(
                      ...
                      child: Icon(Icons.tag_rounded, size: 16, color: accent),
                    ),
                    kOpenHandHGap8,
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            _displayDingTalkDetailLabel(
                              context,
                              kDingTalkDetailConversationIdKey,
                            ),
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                            style: theme.textTheme.labelSmall?.copyWith(
...
class _DingTalkDetailValue extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (flattened is Map) {
      return _DingTalkDetailGrid(data: stringKeyedMapFromValue(flattened));
    }
    ...
    return _DingTalkDetailField(
      label: kDingTalkDetailValueKey,
      value: flattened,
    );
  }
}
...
class _DingTalkDetailField extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return _dingtalkDetailFactSheet(context, [MapEntry(label, value)]);
  }
}
...
Map<String, Object?> _humanizeDingTalkMap(
  ...
  for (final entry in raw.entries) {
    ...
    if (!dingTalkDetailHasContent(valueToShow)) continue;
...
Object? _sanitizeDingTalkDetailValue(Object? value) {
  ...
  if (value is List) {
    return value
        ...
        .where(dingTalkDetailHasContent)
        .toList(growable: false);
  }
  return value;
}
...
bool _dingtalkDetailHasContent(Object? value) =>
    dingTalkDetailHasContent(value);

const Color _kDingTalkDetailViolet = Color(0xFF8B5CF6);
const Color _kDingTalkDetailTeal = Color(0xFF14B8A6);
const Color _kDingTalkDetailRose = Color(0xFFEC4899);

Color _dingtalkDetailSectionAccent(BuildContext context, String title) {
  final colorScheme = Theme.of(context).colorScheme;
  return switch (dingTalkDetailLabelParts(title).stem) {
    '会话概览' => OpenHandStatusColors.info,
    '联系人资料' => colorScheme.primary,
    '员工档案' => colorScheme.tertiary,
    '群聊设置' => OpenHandStatusColors.warning,
    '禁言配置' => OpenHandStatusColors.error,
    '群机器人' => OpenHandStatusColors.success,
    '群身份' || '群成员身份' => _kDingTalkDetailViolet,
    '可见花名册字段' => _kDingTalkDetailTeal,
    '部门资料' || '部门详情' || '组织资料' => _kDingTalkDetailViolet,
...
const double _kDingTalkDetailFactSheetBreakpoint = 520;

Color _dingtalkDetailFactHairline(ColorScheme colorScheme) {
  return colorScheme.outlineVariant.withValues(alpha: 0.58);
}

Widget _dingtalkDetailFactSheet(
  BuildContext context,
  List<MapEntry<String, Object?>> entries,
) {
  if (entries.isEmpty) return const SizedBox.shrink();
  final colorScheme = Theme.of(context).colorScheme;
  final hairline = _dingtalkDetailFactHairline(colorScheme);
  return LayoutBuilder(
    builder: (context, constraints) {
      final maxWidth = constraints.maxWidth.isFinite
          ? constraints.maxWidth
          : _kDingTalkDetailFactSheetBreakpoint;
      final twoColumns = maxWidth >= _kDingTalkDetailFactSheetBreakpoint;
      final rows = <Widget>[];
      ...
      if (!twoColumns) {
        for (final entry in entries) {
          addRow(_dingtalkDetailFactCell(context, entry.key, entry.value));
        }
      } else {
        for (var index = 0; index < entries.length; index += 2) {
          final trailing = index + 1 < entries.length
              ? entries[index + 1]
              : null;
          if (trailing == null) {
            addRow(
              _dingtalkDetailFactCell(
                context,
                entries[index].key,
                entries[index].value,
              ),
...
IconData _dingtalkDetailFactIcon(String stem) {
  return switch (stem) {
    '企业标识' || '企业名称' || '组织' => Icons.apartment_rounded,
    '创建时间' || '操作时间' => Icons.schedule_rounded,
    '钉盘空间标识' || '空间标识' => Icons.cloud_rounded,
    '成员数量' => Icons.groups_rounded,
    kDingTalkDetailConversationIdKey ||
    kDingTalkDetailIdentifierKey => Icons.tag_rounded,
    '群主' || '创建者' || '创建者标识' || '群主标识' => Icons.workspace_premium_rounded,
    kDingTalkDetailTitleNameKey ||
    kDingTalkDetailGroupNameKey ||
    kDingTalkDetailNameKey => Icons.badge_rounded,
    kDingTalkDetailStatusKey => Icons.flag_rounded,
    '群机器人' ||
    '机器人标识' ||
    '机器人编码' ||
    '机器人名称' ||
    '机器人账号' => Icons.smart_toy_rounded,
    '邮箱' || '企业邮箱' => Icons.mail_outline_rounded,
    '手机号' => Icons.phone_rounded,
    '部门' || '所属部门' || '部门标识' => Icons.account_tree_rounded,
    '职位' => Icons.work_outline_rounded,
    _ => Icons.notes_rounded,
  };
}

Widget _dingtalkDetailFactCell(
  BuildContext context,
  String rawLabel,
  Object? value,
) {
  ...
  return Padding(
    ...
    child: Column(
      ...
      children: [
        Row(
          children: [
            Icon(_dingtalkDetailFactIcon(stem), size: 14, color: accent),
            kOpenHandHGap6,
            Expanded(
              child: Text(
...
String _canonicalDingTalkDetailLabel(String key) {
  ...
  if (compact == 'bots' || compact == 'botlist') return '群机器人';
  if (compact == 'creatornick' || compact == 'creator') return '创建者';
  if (compact == 'iscreator') return '是否创建者';
  if (compact == 'manageable' ||
      compact == 'ismanageable' ||
      compact == 'canmanage') {
    return '可管理';
  }
  if (compact == 'openbotid' || compact == 'botid') return '机器人标识';
  if (compact == 'creatorid') return '创建者标识';
  if (compact == 'ownerid') return '群主标识';
  if (compact.startsWith('member')) return '成员资料';
  if (compact.startsWith('department') || compact.startsWith('dept')) {
    return '部门资料';
  }
  return '$kDingTalkDetailExtendedFieldPrefix$key';
}
...
_dingtalkExtendedDetailLabels =
    <String, ({String zhHant, String en, String fr, String de, String ja})>{
      ...
      '类型': (zhHant: '類型', en: 'Type', fr: 'Type', de: 'Typ', ja: '種類'),
      '标识': (
        zhHant: '標識',
        en: 'Identifier',
        fr: 'Identifiant',
      ...
      '项': (zhHant: '項', en: 'items', fr: 'éléments', de: 'Einträge', ja: '項目'),
      '企业标识': (
        zhHant: '企業標識',
        en: 'Organization ID',
        fr: 'Identifiant de l’organisation',
      ...
      '姓名': (zhHant: '姓名', en: 'Name', fr: 'Nom', de: 'Name', ja: '名前'),
      '群内昵称': (
        zhHant: '群內暱稱',
        en: 'Group nickname',
      ...
      '备注': (zhHant: '備註', en: 'Note', fr: 'Note', de: 'Notiz', ja: 'メモ'),
      '状态': (
        zhHant: '狀態',
        en: 'Status',
        fr: 'Statut',
      ...
      '群主标识': (
        zhHant: '群主標識',
        en: 'Owner ID',
        fr: 'Identifiant du propriétaire',
        de: 'Eigentümer-ID',
        ja: 'オーナー ID',
      ),
      '启用': (
...
String _displayDingTalkDetailLabel(BuildContext context, String value) {
  final parts = dingTalkDetailLabelParts(value);
  var stem = parts.stem;
  String withSuffix(String translated) {
    return parts.suffix == null ? translated : '$translated ${parts.suffix}';
  }

  if (stem.startsWith(kDingTalkDetailExtendedFieldPrefix)) {
    return withSuffix(
      dingTalkDetailExtendedFieldLabel(
        context,
        stem.substring(kDingTalkDetailExtendedFieldPrefix.length),
      ),
    );
  }
  if (!dingTalkDetailLabelHasCjk(stem)) {
    stem = _canonicalDingTalkDetailLabel(stem);
    if (stem.startsWith(kDingTalkDetailExtendedFieldPrefix)) {
      return withSuffix(
        dingTalkDetailExtendedFieldLabel(
          context,
  ...
  final extended = _dingtalkExtendedDetailLabels[stem];
  if (extended != null) {
    return withSuffix(
      openHandLocalizedText(
        context,
  ...
  return withSuffix(
    dingTalkDetailLabelHasCjk(stem)
        ? stem
        : dingTalkDetailExtendedFieldLabel(context, stem),
  );
}
...
String _formatDingTalkDetailValue(
  ...
  if (label != null && dingTalkDetailIsFlagLabel(label)) {
    ...
    if (flag != null) {
      ...
      if (stem == kDingTalkDetailNotificationKey) {
        return flag
  ...
  if (label != null) {
    ...
    if (stem == kDingTalkDetailStatusKey) {
      final flag = dingTalkDetailBinaryFlag(value);
      if (flag != null) {
        return flag
            ? dingTalkDetailEnabledLabel(context)
            : dingTalkDetailDisabledLabel(context);
      }
    }
  }
