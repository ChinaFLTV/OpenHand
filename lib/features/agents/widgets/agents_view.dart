...
_Option _agentToolOption(
  ...
  final toolName = _agentToolCanonicalName(kind);
  final summary = _agentToolSummary(context, kind);
  return _Option(id, _agentToolLabel(context, kind), '$toolName · $summary');
}

String _agentToolCanonicalName(AiBuiltinToolKind kind) {
  final name = kind.name;
  if (name.isEmpty) return '';
  return '${name[0].toUpperCase()}${name.substring(1)}';
}

String _agentToolLabel(BuildContext context, AiBuiltinToolKind kind) {
  return switch (kind) {
    ...
    _ => _agentToolCanonicalName(kind),
  };
}

String _agentToolSummary(BuildContext context, AiBuiltinToolKind kind) {
  return switch (kind) {
    AiBuiltinToolKind.agentList => openHandLocalizedText(
      context,
      zh: '发现可用智能体',
