...
class HarnessSessionStore {
  ...
  ///
  /// Throws when the stored payload is malformed so callers can surface data
  /// corruption instead of treating it as a missing session.
  Future<HarnessSessionRecord?> load() {
    return _operations.enqueue(() async {
      ...
      if (dataJson == null) {
        throw const FormatException('Harness session payload is missing.');
      }
      final decoded = optionalStringKeyedMapFromJsonText(dataJson);
      if (decoded == null) {
        throw const FormatException('Harness session payload is malformed.');
      }
