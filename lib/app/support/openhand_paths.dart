...
abstract final class OpenHandPaths {
  ...
  static String? environmentHomeDirectoryPath() {
    final home = nullIfBlank(currentPlatformEnvironmentValue('HOME'));
    if (home != null && p.isAbsolute(home)) {
      return _normalizeHomePath(home);
    }

    final userProfile = nullIfBlank(
      currentPlatformEnvironmentValue('USERPROFILE'),
    );
    if (userProfile != null && p.isAbsolute(userProfile)) {
      return _normalizeHomePath(userProfile);
    }

    final homeDrive = nullIfBlank(currentPlatformEnvironmentValue('HOMEDRIVE'));
    final homePath = nullIfBlank(currentPlatformEnvironmentValue('HOMEPATH'));
