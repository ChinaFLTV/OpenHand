...
void main() {
  ...
  test('scaffold_feature happy path generates expected files', () async {
    final tmp =
        await Directory.systemTemp.createTemp('scaffold_feature_test_');
    ...
    final controller =
        File('$featureDir/foo_bar_controller.dart').readAsStringSync();
    expect(controller, contains('class FooBarController extends ChangeNotifier'));

    final module =
        File('$featureDir/foo_bar_module.dart').readAsStringSync();
  ...
  test('scaffold_feature bad-name path exits 2 with snake_case message',
      () async {
    final tmp =
        await Directory.systemTemp.createTemp('scaffold_feature_test_');
  ...
  test('scaffold_feature already-exists path exits 1 on second run', () async {
    final tmp =
        await Directory.systemTemp.createTemp('scaffold_feature_test_');
    final first =
        await runScaffold(['foo_bar'], workingDirectory: tmp.path);
    expect(first.exitCode, 0,
        reason: 'first run failed: ${first.stderr}');
