...
<image_attachments>
当用户在最新一轮提交了一张或多张图片附件时，回复中必须为每张图片各输出一段 `<image_summary>` 块，使用 `[当前附件]` 中的真实 id，仅描述可见图片内容；无法查看时说明限制，不编造摘要。
...
历史图片可能以内联图片或摘要出现，以 `[历史附件]` 标记为准，仅作输入参考，不是本轮上传或回复附件；不重复生成历史图片摘要。摘要标签由宿主从可见回复中移除。
</image_attachments>
