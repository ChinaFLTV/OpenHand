...
- 浏览器 tab 提供：tab strip（多 page target 切换 / 关闭 / 新建）、地址栏 + 前进 / 后退 / 刷新、缩放下拉（50%–150%）、保存当前帧、键鼠 + IME 输入桥、右键菜单（复制 / 粘贴 / 全选 / 刷新 / 检查元素 / 外部打开）。所有交互都通过 CDP `Input.*` / `Page.*` / `Emulation.*` 真实下发。
