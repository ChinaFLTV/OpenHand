...
- 应用 tab 支持 Cookies / LocalStorage / SessionStorage 编辑（新增 / 修改 / 删除）+ Service Worker 注册 / 更新 / 卸载；记录器 tab 支持一键导出为 puppeteer / playwright JS 脚本；高级菜单新增「网络拦截规则」入口（URL 通配 → block / 重写 URL / 注入 Header，规则持久化到 session metadata）；Network 面板单条请求右键「编辑后重放」可临时改写 URL / Headers 再 replay，工具栏「批量操作」按钮按当前过滤结果一次性 block / replay / 复制 curl，工具栏「HAR 对比」按 method+url+status 三元组对两份 .har 做请求级集合 diff；Performance 面板一键导出 FPS + Long task 历史 CSV，并把最近一次 Tracing.dataCollected 渲染为可缩放火焰图（按 tid 分行、慢任务高亮）；Memory 面板每次采集自动滚动 A/B 两个槽位，「比较快照」用 isolate 解析 .heapsnapshot 的 nodes/strings 表，按 constructor 聚合自有大小，列出 Top 40 构造器字节增长；高级菜单「WebRTC 实时面板」按 PeerConnection id 维护 60 点环形采样，1 秒一次 getStats 渲染 bytesSent / bytesReceived / packetsLost / RTT 折线；Sources 面板支持跨脚本代码搜索（grep 全部已缓存源码）和断点持久化，浏览器重启自动复原；可选 LSP（chip 启用，默认拉起 typescript-language-server --stdio），右键代码行触发 hover / 跳转定义 / 重命名预览，未安装时自动退化不影响基础功能；Console REPL 历史按会话持久化，上下箭头浏览。
