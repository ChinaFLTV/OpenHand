...
- 应用 tab 支持 Cookies / LocalStorage / SessionStorage 编辑（新增 / 修改 / 删除）；记录器 tab 支持一键导出为 puppeteer / playwright JS 脚本；高级菜单新增「网络拦截规则」入口（URL 通配 → block / 重写 URL / 注入 Header）；Network 面板单条请求右键「编辑后重放」可临时改写 URL / Headers 再 replay；Performance 面板一键导出 FPS + Long task 历史 CSV；Sources 面板的断点持久化到 session metadata，浏览器重启自动复原；Console REPL 历史按会话持久化，上下箭头浏览。
