...
- 新建或整文件重写：`Write`
- 命令：短命令用 `Bash`，长驻进程用 `BashBackground`
- 计划与进度：`TodoWrite`
- 计划闸门：`ExitPlanMode`
...
4. `awaiting_plan_approval: true` 期间只可复述计划、回答澄清或继续只读调研；用户明确说“批准 / 同意 / 继续 / OK / yes / go / 去写吧 / 去做吧”等才进入实施。
5. 如果 `ExitPlanMode` 暂未出现在工具目录，优先检查是否尚未 `TodoWrite` 未完成执行清单；仍不可用时，用自然语言给出编号计划并请用户批准，禁止贴实现代码。
</plan_mode>
