...
import { decisionJsonDraft, initialDecisionDraft, DECISION_MAX_CHARACTERS, decisionDraft, type DecisionType } from '../shared/util/decision';
  ...
  return <DialogFrame closing={closing} onRequestClose={requestClose} ariaLabel="配置结构化决策"
    ...
    <div class="overflow-auto min-h-0 p-5 space-y-4">
      ...
      {initial.advanced === undefined && <div class="flex flex-wrap gap-2" role="group" aria-label="决策类型">{(['noul', 'choice', 'score'] as const).map((value) =>
        <button type="button" class="oh-decision-type oh-tap-press" aria-pressed={type === value} style={{ background: type === value ? 'var(--m3-primary-container)' : undefined }} onClick={() => setType(value)}>{({noul: '判断', choice: '选择', score: '评分'})[value]}</button>)}</div>}
