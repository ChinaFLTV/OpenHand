...
import { t } from '../i18n';
import { decisionJsonDraft, initialDecisionDraft, DECISION_MAX_CHARACTERS, DECISION_TYPES, decisionDraft, decisionQuestionForType, type DecisionType } from '../shared/util/decision';
...
export function DecisionRequestDialog({ initialText, onApply, onClose }: { initialText: string; onApply: (text: string) => void; onClose: () => void }) {
  ...
  return <DialogFrame closing={closing} onRequestClose={requestClose} ariaLabel={t('decision.dialog.title', '配置结构化决策')}
    ...
    <div class="overflow-auto min-h-0 p-5 space-y-4">
      ...
      {initial.advanced === undefined && <div class="oh-decision-type-group" role="group" aria-label={t('decision.field.type', '决策类型')}>{DECISION_TYPES.map((value) =>
        <button type="button" class="oh-decision-type" aria-pressed={type === value} onClick={() => updateType(value)}>{t(`decision.type.${value}`, value === 'noul' ? '判断' : value === 'choice' ? '选择' : '评分')}</button>)}</div>}
      <label class="block">{initial.advanced !== undefined ? t('decision.field.advancedJson', '完整决策配置（JSON）') : t('decision.field.state', '待评估内容')}<textarea class="oh-decision-input" rows={initial.advanced !== undefined ? 14 : 4} maxLength={DECISION_MAX_CHARACTERS} value={state} onInput={(event) => setState(event.currentTarget.value)} /></label>
      {initial.advanced === undefined && <><label class="block">{t('decision.field.question', '需要模型回答的问题')}<textarea class="oh-decision-input" rows={2} value={question} onInput={(event) => setQuestion(event.currentTarget.value)} /></label>
      {type !== 'noul' && <label class="block" key={type}>{type === 'choice' ? t('decision.field.choiceLines', '候选项，每行一个') : t('decision.field.scoreLines', '评分等级，从低到高每行一个（2—10 级）')}<textarea class="oh-decision-input" rows={4} value={criteria} onInput={(event) => { const value = event.currentTarget.value; setCriteriaByType(current => ({ ...current, [type]: value })); }} /></label>}
      </>}
