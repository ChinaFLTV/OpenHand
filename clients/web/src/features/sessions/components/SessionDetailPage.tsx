...
import { MachineTerminalPanel } from './MachineTerminalPanel';
...
export function SessionDetailPage() {
  ...
  const handleToggleMessageTranslation = useEventCallback(async (m: SessionMessage) => {
    ...
    if (isStructuredDecisionMessage(m)) return;
  ...
  const handleToggleMessageTts = useCallback(async (m: SessionMessage) => {
    if (isToolMessage(m) || isStructuredDecisionMessage(m)) return;
    if (!sessionId) return;
    const requestSessionId = sessionId;
    try {
  ...
  function messageWindowHasNewerMessages(): boolean {
    return hasNewerMessagesRef.current;
  }

  function updateHasNewerMessages(value: boolean): void {
    hasNewerMessagesRef.current = value;
    setHasNewerMessages(value);
  }
  ...
  function loadDetail(): void {
    ...
    Promise.all([
      ...
      .then(([d, m]) => {
        ...
        restoreMessageWindow(m.items, m.offset);
        updateHasNewerMessages(m.has_newer ?? false);
  ...
  async function loadOlder(): Promise<void> {
    ...
    const boundaryId = messagesRef.current[0]?.id;
    ...
    try {
      ...
      updateTotalKnown(Math.max(totalKnownRef.current, m.total));
      schedulePostRenderFrame(() => {
        if (!ownsSessionAsyncResult(requestSessionId) ||
            lastUserScrollIntentAtRef.current !== scrollIntentAt) return;
        const el = mainRef.current;
        if (!el) return;
      ...
      if (handleAuthError(e)) return;
      if (handleSessionGoneError(e)) return;
      showSnackbar(e instanceof Error ? e.message : String(e), { tone: 'error' });
    } finally {
    ...
    try {
      ...
      if (!targetFound) {
        pageResult = await listMessages(requestSessionId, {
          limit: MESSAGE_LIST_REVEAL_PAGE_SIZE,
      ...
      if (pageResult != null) {
        ...
        restoreMessageWindow(pageResult.items, pageResult.offset);
        updateHasNewerMessages(pageResult.has_newer ?? pageResult.has_more);
      ...
      >
        ...
        <SessionTopBar
          ...
          trailing={
            <button
              type="button"
              onClick={() => void (hasNewerMessages ? returnToLatest() : refresh())}
        ...
        <div class="flex flex-col gap-4">
          ...
          {Object.keys(rehydration).length > 0 ? (
            <Section title="压缩后上下文恢复" accent={DIALOG_ACCENT.tertiary} icon={<DialogGlyph name="spark" />}>
              ...
              <EntryRow label={metadataFieldLabel('restored_signal_counts')} value={`read_files=${integerFromUnknown(rehydration['recent_read_file_count'])}, skills=${integerFromUnknown(rehydration['invoked_skill_count'])}, mcp_instructions=${integerFromUnknown(rehydration['mcp_server_instruction_count'])}, session_hooks=${integerFromUnknown(rehydration['session_start_hook_count'])}, subagent_results=${integerFromUnknown(rehydration['subagent_result_count'])}, deferred_tools=${integerFromUnknown(rehydration['deferred_builtin_tool_count'])}, subagent_types=${integerFromUnknown(rehydration['subagent_type_count'])}`} />
